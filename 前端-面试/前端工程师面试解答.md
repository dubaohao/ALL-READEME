...
![img](F:\Code\github\前端-面试\assets\20180718200247892.png)

> CMD-通用模块定义

**CMD---**是**SeaJS**在推广过程中对模块定义的规范化产出，是一个**同步模块定义**，是SeaJS的一个标准，SeaJS是CMD概念的一个实现，SeaJS是淘宝团队提供的一个模块开发的js框架.

**通过define()定义，没有依赖前置，**通过require加载jQuery插件**，CMD是依赖就近**，在什么地方使用到插件就在什么地方require该插件，即用即返，这是一个同步的概念

![img](F:\Code\github\前端-面试\assets\20180718203155130.png)
...
![img](F:\Code\github\前端-面试\assets\20180718203952178.png)

> **ES6特性，模块化**---**export/import对模块进行导出导入的**

![img](F:\Code\github\前端-面试\assets\20180718204535949.png)

>  **RequireJS 和 Sea.js** 对比
...
6. CMD 推崇**依赖就近**，AMD 推崇**依赖前置。**在AMD中，我们需要把依赖前置在依赖数组中。而在cmd中，我们只需要在使用这个模块前，把依赖的模块require进来。
...
**async 和 await**Generator函数的语法糖，是的异步操作变得更加方便。一个函数如果加上 `async` ，那么该函数就会返回一个 `Promise`
